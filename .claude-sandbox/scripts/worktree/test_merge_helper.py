...
class TestComposeProjectName(unittest.TestCase):
    ...
    def test_default_returns_base_name(self):
        result = self._run("checkpoint-sampler-dev")
        self.assertEqual(result.returncode, 0)
        self.assertEqual(result.stdout.strip(), "checkpoint-sampler-dev")

    def test_story_id_scopes_name(self):
        result = self._run("checkpoint-sampler-dev", {"STORY_ID": "S-042"})
        self.assertEqual(result.returncode, 0)
        self.assertEqual(result.stdout.strip(), "checkpoint-sampler-dev-s-042")
    ...
    def test_story_id_normalized(self):
        result = self._run("checkpoint-sampler-test", {"STORY_ID": "W-024"})
        self.assertEqual(result.returncode, 0)
        self.assertEqual(result.stdout.strip(), "checkpoint-sampler-test-w-024")
