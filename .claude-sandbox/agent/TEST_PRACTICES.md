...
#### 2.1.1 Mockery generation workflow (mocks for interfaces)
Mocks for Go interfaces are **generated by mockery**, not hand-rolled. This
prevents drift where a stale hand-rolled mock keeps passing while the interface
it stands in for changes.

- **Config**: `backend/.mockery.yaml` (mockery v3 schema). It lists each
  consumer package and the interfaces to mock.
- **Pinned version**: `v3.7.1`, pinned in `backend/Makefile` (`MOCKERY_VERSION`).
  Invoked via `go run github.com/vektra/mockery/v3@<version>` so no container
  rebuild is needed to regenerate locally.
- **Generate command**: from `backend/`, run `make mocks`.
- **Where generated mocks live**: a dedicated, predictable package per layer
  (DEVELOPMENT_PRACTICES 3.9), e.g. `backend/internal/api/apimocks`. Generated
  files are named `mock_<Interface>.go`, carry a `// Code generated ... DO NOT
  EDIT.` header, and are committed. Test packages import the mocks package and
  use the testify-style API (`m.EXPECT().Method(args).Return(...)` /
  `.RunAndReturn(fn)`).
- **Determinism**: `make mocks` is deterministic — running it twice produces no
  git diff. After changing an interface (or `.mockery.yaml`), re-run `make
  mocks` and commit the regenerated files.
- **Codegen ordering**: Goa codegen runs before mocks when mocking generated
  types. Mockery is configured only for interfaces defined in hand-written
  consumer packages; do not point it at `internal/api/gen` (generated and
  gitignored).
