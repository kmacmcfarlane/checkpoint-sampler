...
- **Reason**: Vite proxies WebSocket connections (`ws: true`) from `localhost:3000/api` to `backend:8080`. During E2E test startup the frontend container starts before the backend is fully accepting connections, causing brief ECONNREFUSED errors until the backend is ready. During teardown the backend container stops first; any in-flight WebSocket upgrade or proxy write attempt from Vite will fail with EPIPE or ECONNREFUSED. Both are timing artifacts of the Docker Compose start/stop sequence and are not indicative of application bugs. The application functions correctly once both services are running and does not attempt to reconnect via Vite proxy (the frontend WebSocket client reconnects directly).
- **Reference**: W-006
