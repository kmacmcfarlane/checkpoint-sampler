...
- If no eligible stories remain across any queue, touch `.ralph/stop` and exit with code `0`. Note: `uat` stories without `uat_feedback` are not eligible work — the ralph loop will pick up the next story in a fresh iteration.
