...
// Directory structure:
//
//	sample_dir/{study_name}/{checkpoint_filename.safetensors}/ → study-scoped samples
...
// StudyNameForRun extracts the study name prefix from a viewer-discovered training run name.
...
func StudyNameForRun(runName string) string {
	...
	// - "study_name/base_name" → study_name
	// - "base_name" → "" (legacy)
	// The base_name itself could contain "/" if the directory structure had nested dirs,
	// but in practice studies are single-level directories under sample_dir.
