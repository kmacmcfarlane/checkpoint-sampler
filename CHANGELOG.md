...
- `frontend/src/App.vue`: Added `eagerAutoSelect()` function that eagerly loads saved training run from localStorage on mount (independent of drawer state), and added idempotency guard to `onTrainingRunSelect` to prevent redundant scans when the same run is re-selected; (UAT rework) removed `hasSamples: true` filter from `eagerAutoSelect()` so training runs without samples are also found
- `frontend/src/__tests__/App.test.ts`: Added 8 new tests in "Eager auto-select on narrow screens" describe block covering eager loading, header button visibility, no-data fallback, stale data handling, drawer sync, idempotency, wide screen behavior, and API failure graceful handling; (UAT rework) added regression test for `has_samples: false` training runs
- `frontend/e2e/narrow-screen-eager-select.spec.ts`: (UAT rework) 3 new E2E tests covering narrow-screen eager auto-select, header button visibility, and drawer sync
- 727 frontend tests pass; 564 backend tests pass; 38 E2E tests pass
