...
### S-180: Gate dependency audit at commit time instead of push time
- Ralph-loop agents commit freely and only push/merge later, so a `pre-push` audit hook never actually gated agent work — a story could be finished, committed, and merged with unresolved govulncheck / npm-audit findings. Moved the gate to `pre-commit` so no commit can land while high-severity vulnerabilities are present
- `scripts/git-hooks/pre-push` renamed to `scripts/git-hooks/pre-commit` (still `make audit` under `set -euo pipefail`); `make install-hooks` continues to wire it via `core.hooksPath=scripts/git-hooks` and is idempotent. Running the full audit per commit is the accepted tradeoff
- The existing offline soft-skip is preserved unchanged: when the vulnerability DB is unreachable the hook prints the loud warning banner and exits 0 rather than blocking the commit. Makefile, README, and `scripts/audit.sh` wording updated from push- to commit-time
- Turning the hook on immediately proved the point: the very first commit attempt was blocked by a real high-severity finding (GO-2026-5970, infinite loop on invalid input in `golang.org/x/text`, reachable from the Goa design DSL). Bumped `golang.org/x/text` v0.33.0 → v0.39.0 (pulling `net` v0.56.0, `sync` v0.21.0, `sys` v0.46.0, `tools` v0.47.0) so the audit passes; backend suite 1246 specs green after the bump. Under the old pre-push hook this would have ridden along to main unnoticed
- Also folded in an unrelated E2E test correction found during the QA gate: `seed-jobs.spec.ts` asserted a freshly-seeded job stays `pending`, but the executor's 1s ticker legitimately auto-starts the oldest pending job, so the assertion now accepts `pending` or `running`. `scripts/test-audit.sh` 7/7, E2E 429/429, sweep clean
...
- All image URL construction now routes through a shared `frontend/src/api/urls.ts` helper instead of four independent raw-string sites; `GetItemCounts` failures in the API layer log at warn instead of silently reporting zero counts
- AC asked for jobProgress entries to be deleted on terminal status; deleting them outright would have regressed the completion summary `JobProgressItem` renders for finished jobs. Instead live-only fields are stripped (`current_sample_params` holds full prompt text and is the real memory weight) and orphaned entries pruned against the live job list — the actual unbounded-growth path. Dead `ComboFilter.vue` and its test removed. Backend 1511 specs, frontend 2044, E2E 429/429, sweep clean
...
- Collapsing three near-duplicate prompt filters into one shared `isValidPrompt` helper silently changed a user-visible number: two originals trimmed, one used a truthiness check, so whitespace-only prompts stopped counting toward the total-image estimate. Caught in review and reverted at that one site, with the asymmetry now commented and pinned by a regression test. The arguably-better unified behavior was left as a separate decision rather than an incidental side effect
- Scoped CSS silently stops applying when markup moves from a parent to a child component, and no existing gate could catch it — jsdom applies no stylesheets and `vue-tsc` does not resolve CSS against markup, so a broken layout would have shipped fully green. Added `frontend/e2e/scoped-styles-r021.spec.ts` asserting computed styles on non-root descendants, verified non-vacuous by deliberately breaking a selector and confirming failure. Full E2E: 429/429
...
- `serve` takes an injected listener so shutdown tests bind `127.0.0.1:0` rather than a fixed port. 21 new specs cover log-level resolution, closer ordering and idempotence, component composition, two wiring-failure paths, and three graceful-shutdown behaviors
- `backend/internal/model/study_lora_test.go` adds DescribeTable coverage for `ImagesPerCheckpointLoRA`, `TooManyItemsError.Error/Code`, `KnownCSRoles`, and `IsKnownRole` — helpers shared across the service and store interfaces, where a silent semantic change hits both layers at once
- Coverage: `cmd/server` 6.4% → 71.1%, `internal/model` 57.7% → 100.0%. The residual `cmd/server` gap is `main()`, `run()`'s env/signal handling, the OpenAPI path probes, and the ComfyUI-enabled happy path
- One real semantic change on the live path: the listener is now created before `signal.NotifyContext` rather than binding inside `ListenAndServe` after it. Verified shutdown-equivalent against the stdlib — `http.Serve` defers `l.Close()` before `trackListener`, so a `Shutdown` that wins the startup race still closes the listener. The only delta is that a SIGTERM arriving between `net.Listen` and `NotifyContext` uses default disposition, a window far smaller than the pre-existing unguarded span across `buildServer`
- Behavior-preserving refactor plus tests; no user-facing change. Confirmed by a full E2E run at 427/427 with clean container start and teardown on all 4 shards
- A pre-existing shutdown race was found during review and filed as B-174 rather than fixed here: `serve`'s non-`ErrServerClosed` error path returns without draining `shutdownDone`, so worker `Stop()` can run concurrently with the DB close. The hole existed byte-for-byte before this refactor, and did not manifest in E2E

### W-031: Deterministic synchronization replacing fixed-timeout waits in tests
- `frontend/e2e/helpers.ts` is now free of `page.waitForTimeout` (6 sites removed). Because `closeDrawer`, `closeFiltersDrawer`, and the select-popup retry paths are used by nearly all 83 specs, these sleeps were the project's largest flake multiplier — the source of the B-108/B-111/B-112/B-128/B-134 family of bugs
- Drawer teardown now waits for `.n-drawer-mask:visible` to reach count 0 rather than sleeping 300ms. Naive UI renders the mask inside a Vue `<Transition>`, so it unmounts once the leave transition finishes; because Playwright's `:visible` is bounding-box-based rather than opacity-based, the wait ends at exactly the moment the mask stops intercepting pointer events. This is both more correct and more tolerant than the fixed delay it replaces (5s budget vs. a hard 300ms ceiling)
- Spec-level waits replaced with polls on the state actually being awaited: image `src` transitions in `lightbox-slider-sync.spec.ts`, and `expect.poll` in `job-delete.spec.ts` in place of a manual 10×3s refresh loop. The 3s WebSocket-propagation sleep in `sample-generation.spec.ts` was removed outright — the following `pollJobStatus` call was already the real synchronization point
- Backend: both 30ms `time.Sleep` calls in `comfyui_client_test.go`'s caller-cancellation tests replaced with a `received` channel closed on handler entry (guarded by `sync.Once`, since the client may retry). Cancellation now fires at the precise moment the request is in-flight instead of at a guessed offset
- TEST_PRACTICES 6.9 previously *prescribed* `waitForTimeout(300)` for the mask animation; it now prescribes the detach assertion and points at the shared helpers. 6.10 was expanded to require an inline comment marking each legitimate hold-position wait as a documented exception, making the rule mechanically checkable
- One fixed wait is retained by design: the hold-position negative check in `slider-playback.spec.ts`, where there is no state transition to await and real time must pass before asserting nothing moved. 34 further spec-level waits remain outside this story's scope (tracked in `agent/ideas/testing.md`)
- Verified by two consecutive full E2E runs at 427/427 with zero flakes, plus `ginkgo --race --repeat=4` on the touched package
