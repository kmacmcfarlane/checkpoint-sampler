...
describe('usePresetPersistence', () => {
  ...
  // S-155: training-run ids are stable opaque strings; combo keys are built from
  // string ids. The legacy numeric single-entry migration was removed.

  it('returns null when no data is stored for a combo', () => {
    const { getPresetIdForCombo } = usePresetPersistence()
    expect(getPresetIdForCombo('run-1', '')).toBeNull()
    expect(getPresetIdForCombo('run-1', 'study-a')).toBeNull()
  })
  ...
  it('saves and retrieves preset per training run + study combo', () => {
    ...
    savePresetSelection('run-123', 'study-a', 'preset-abc')
    expect(getPresetIdForCombo('run-123', 'study-a')).toBe('preset-abc')
  })
  ...
  it('saves preset to localStorage', () => {
    ...
    expect(stored.presetsByKey['run-123|study-a']).toBe('preset-abc')
  })
  ...
  it('different TR+study combos are stored independently', () => {
    ...
    savePresetSelection('run-1', 'study-a', 'preset-1a')
    savePresetSelection('run-1', 'study-b', 'preset-1b')
    savePresetSelection('run-2', 'study-a', 'preset-2a')

    expect(getPresetIdForCombo('run-1', 'study-a')).toBe('preset-1a')
    expect(getPresetIdForCombo('run-1', 'study-b')).toBe('preset-1b')
    expect(getPresetIdForCombo('run-2', 'study-a')).toBe('preset-2a')
  })
  ...
  it('returns null for a combo that has not been saved', () => {
    ...
    savePresetSelection('run-1', 'study-a', 'preset-abc')
    expect(getPresetIdForCombo('run-2', 'study-a')).toBeNull()
    expect(getPresetIdForCombo('run-1', 'study-b')).toBeNull()
  })

  it('overwrites previous preset for the same TR+study combo', () => {
    const { savePresetSelection, getPresetIdForCombo } = usePresetPersistence()

    savePresetSelection('run-1', 'study-a', 'preset-old')
    savePresetSelection('run-1', 'study-a', 'preset-new')

    expect(getPresetIdForCombo('run-1', 'study-a')).toBe('preset-new')
  })
  ...
  it('clears preset for a specific TR+study combo', () => {
    ...
    savePresetSelection('run-1', 'study-a', 'preset-abc')
    savePresetSelection('run-2', 'study-a', 'preset-xyz')

    clearPresetForCombo('run-1', 'study-a')

    expect(getPresetIdForCombo('run-1', 'study-a')).toBeNull()
    // Other combos should be unaffected
    expect(getPresetIdForCombo('run-2', 'study-a')).toBe('preset-xyz')
  })
  ...
  it('clears all preset selections', () => {
    ...
    savePresetSelection('run-1', 'study-a', 'preset-abc')
    savePresetSelection('run-2', 'study-b', 'preset-xyz')

    clearAllPresetSelections()

    expect(getPresetIdForCombo('run-1', 'study-a')).toBeNull()
    expect(getPresetIdForCombo('run-2', 'study-b')).toBeNull()
  ...
  it('supports empty studyOutputDir as a valid combo key', () => {
    ...
    savePresetSelection('run-1', '', 'preset-no-study')
    expect(getPresetIdForCombo('run-1', '')).toBe('preset-no-study')
    // Should not match study variants
    expect(getPresetIdForCombo('run-1', 'some-study')).toBeNull()
  })
  ...
  it('restores data from localStorage on next instantiation', () => {
    ...
    const { savePresetSelection } = usePresetPersistence()
    savePresetSelection('run-42', 'study-x', 'preset-saved')

    // Simulate new page load — new composable instance reads from localStorage
    const { getPresetIdForCombo } = usePresetPersistence()
    expect(getPresetIdForCombo('run-42', 'study-x')).toBe('preset-saved')
  })
  ...
  it('returns null for unknown combo even when localStorage has data for other combos', () => {
    ...
    savePresetSelection('run-1', 'study-a', 'preset-abc')
    expect(getPresetIdForCombo('run-99', 'study-z')).toBeNull()
  })
