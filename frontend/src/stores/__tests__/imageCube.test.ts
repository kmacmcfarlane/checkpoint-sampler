...
describe('useImageCubeStore', () => {
  ...
  describe('image-cube indexing', () => {
    ...
    it('maps each slider value to an image URL for a cell', () => {
      ...
      // R-022: path segments are percent-encoded; Goa's mux PathUnescapes them
      // back to the raw filename server-side.
      expect(bySlider['42']).toBe('/api/v1/images/run/step%3D500%26cfg%3D7%26seed%3D42.png')
    })
    ...
    it('tracks the focused cell and derives its image URL', () => {
      ...
      expect(store.focusedImage).toBe('/api/v1/images/run/step%3D500%26cfg%3D7%26seed%3D42.png')
